...
| R02 | **Fabricated / ghost entity submissions** -- phantom POIs with random storefront photos | 4-High | 5-Critical | **20** | Photo evidence required for all submissions; perceptual near-duplicate detection v2 (pHash + dHash AND-rule, full-corpus indexed lookup — see Mitigation Changelog 2026-07); entity proximity thresholds per vertical (e.g., 10m for mobile_money); spot-check audits (10--50 POIs/week); XP escrowed until admin review | Charles | Audit reveals >20% of sampled POIs do not exist at stated location; photo hashing flags >3 near-identical photos from same agent | Immediate: halt agent submissions; conduct field verification of all agent's recent POIs; if confirmed, terminate agent |
| R10 | **Recycled / stock photos** -- reusing one trip's photos for multiple submissions | 4-High | 4-High | **16** | SHA-256 exact hash matching; perceptual near-duplicate detection v2 (pHash + dHash, robust to re-encoding/screenshots; cross-point matches add +30 photoRisk before scoring; same-point reuse exempt by policy); EXIF fingerprint matching (same device + time + GPS = flag); screenshot detection (no EXIF + specific resolution patterns); semantic embedding match (Stage B, dark-launched) for crop/angle variants | Charles | Photo hash system flags >2 near-identical photos from same agent in one day | Review agent's submission batch; if confirmed recycling, reject all and issue warning; second offense = termination |
...
## 4. Mitigation Changelog

### 2026-07 — Image similarity v2 + AI cost controls (PRs #40, #42)

Guardrails shipped after the pilot, superseding the aHash-based photo deduplication referenced in R02/R10:

- **Stage A perceptual near-duplicate detection (live).** Every submission photo gets a DCT pHash + gradient dHash. A match on a *different* point (pHash Hamming ≤ 8 AND dHash ≤ 6) adds +30 photoRisk and a `perceptual_near_duplicate` review flag. Robust to re-encoding and screenshots, which defeat SHA-256 matching. Replaces the aHash comparison that only scanned the 100 most recent photos — coverage no longer degrades as the corpus grows (pigeonhole segment indexes, full-corpus lookup at constant cost). Directly mitigates R02 and R10.
- **Scoring-order fix.** The near-duplicate risk bump previously applied *after* the final risk score and XP tier were computed, so it never affected auto-approval or payout. It now runs before scoring and actually gates review status and XP.
- **Historical backfill (completed 2026-07-05).** All 108 pre-upgrade photos were re-fingerprinted, so the duplicate check covers the entire photo history, not just post-upgrade submissions.
- **Same-point reuse policy.** An ENRICH may reuse the CREATE photo on the same point without penalty (confirmed product decision, 2026-07-05). Only cross-point reuse is treated as a fraud signal.
- **Stage B semantic matching (dark-launched).** Vertex multimodal embeddings + pgvector ANN can catch crop/angle/filter variants that pHash misses and retroactively flip `auto_approved` → `pending_review` within 24h. Inactive until `IMAGE_EMBEDDING_ENABLED=true` + GCP credentials are set.
- **AI kill switch (PR #42).** Edge Config flag `ai_mode` (`npm run ai:mode -- deterministic|gemini`) toggles all Gemini LLM usage at runtime with no redeploy — caps AI cost/exposure instantly. Currently set to `deterministic`.
