...
const SCREEN_LIBRARY: Record<string, DocsScreen> = {
  ...
  'admin-account-access': {
    ...
    summary: 'Lets admins look up any user account by email or phone and change their role between Agent, Client, and Admin. Promoting a user to Admin automatically unlocks the worldwide map scope.',
    whyItMatters: 'Role management is the trust escalation lever. Only admins can grant admin access, and every change is audit-logged with before/after state for security review.',
    primaryActions: [
      'Search for an account by exact email or phone number.',
      'Review the account current role and map scope.',
      'Change the role to Agent, Client, or Admin and save.',
    ],
    keySignals: [
      'Current role and map scope of the target account',
      'Confirmation message after a successful role change',
      'Self-edit protection: admins cannot change their own role from this panel',
    ],
    screenshots: [],
  },
  'admin-map-scope-toggle': {
    id: 'admin-map-scope-toggle',
    title: 'Map scope toggle',
    surface: 'Admin field orchestration',
    audiences: ['admin'],
    entryPoint: 'Home explorer header (admin only)',
    summary: 'Admins can switch the map view between Bonamoussadi (pilot zone), Cameroon, and Worldwide. The selection persists across sessions and controls which data points are fetched and displayed.',
    whyItMatters: 'Field operations start in a restricted pilot zone, but admins need to see the full picture when reviewing nationwide or global data quality and coverage.',
    primaryActions: [
      'Toggle between Bonamoussadi, Cameroon, and Worldwide views.',
      'Observe how data points, map center, and zoom level adapt to the selected scope.',
      'Verify that non-admin users remain locked to Bonamoussadi.',
    ],
    keySignals: [
      'Active scope button state (highlighted in navy)',
      'Map re-centers and adjusts zoom on scope change',
      'Scope selection remembered on next visit',
    ],
    screenshots: [],
  },
  'settings-profile': {
...
export const DOCS_SECTIONS: Record<DocsAudience, DocsSection> = {
  ...
  admin: {
    ...
    workflows: [
      ...
      {
        id: 'admin-access-and-scope',
        title: 'Manage account roles and map scope',
        summary: 'Promote users to admin, change roles, and toggle the map between Bonamoussadi, Cameroon, and Worldwide.',
        steps: [
          'Open Profile and scroll to the Account Access card.',
          'Look up a user by exact email or phone number.',
          'Change their role to Admin (or back to Agent/Client) and save.',
          'Return to the Home explorer and use the Map Scope toggle to switch between Bonamoussadi, Cameroon, and Worldwide.',
        ],
    ...
    runbook: [
      ...
      {
        title: 'Account and scope management',
        items: [
          'Promote a user to admin from Profile > Account Access using their exact email or phone.',
          'Admin promotion automatically sets `mapScope` to `global` and `isAdmin` to `true`.',
